...
**Acceptance Criteria**:
- [ ] Lesson cards in curriculum view display completion status (not started / started / completed)
- [ ] Completed lessons show score badge with color coding (≥90% blue, ≥80% green, <80% yellow, <60% red)
- [ ] Badge displays most recent score percentage
- [ ] Hover tooltip shows: attempt count, most recent score, best score
- [ ] "Start Quiz" button appears at end of lesson content when no attempts exist
- [ ] "Retake Quiz" button appears when at least one attempt exists
- [ ] Clicking quiz button navigates to quiz interface
- [ ] After quiz submission, student returns to lesson view with updated score badge
- [ ] API endpoint `GET /api/students/{studentId}/lessons/{lessonId}/progress` returns completion data
- [ ] Completion status updated automatically after quiz submission (any score = completed)

**Test Plan**:
- Integration test: Verify completion status after first quiz attempt
- Integration test: Verify score badge updates after retake
- Integration test: Verify attempt count increments
- Manual test: Complete lesson quiz, verify badge appears in curriculum view
- Manual test: Retake quiz, verify badge updates with new score
- Manual test: Hover over badge, verify tooltip shows attempt history
